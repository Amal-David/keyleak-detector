...
| W5 | S1 | **No CI; 4 test suites silently disabled.** `.github/workflows/` absent; `tldextract` missing from `requirements.txt` aborts `test_site_scanner{,_ssrf}`, `test_subdomain_takeover`, `test_dangerous_url_params` before they run — so crawler scope/SSRF enforcement is effectively **untested in practice**. | no workflow dir; `pyproject.toml:32-34` vs `requirements.txt`; suite: `errors=4` | ✅ (ran suite) |
