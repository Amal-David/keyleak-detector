...
class GhActionsHardeningTests(unittest.TestCase):
    ...
    def test_detectors_are_repo_only_not_extension(self):
        ...
        for det in DETECTORS:
            if det.id.startswith("gh_actions_") and det.id != "gh_actions_pull_request_target":
                pass  # presence check below
        ghs = {d.id: d for d in DETECTORS if d.id in (
            "gh_actions_unpinned_action", "gh_actions_write_all_permissions",
            "gh_actions_secret_echoed_in_run")}
        self.assertEqual(len(ghs), 3)
